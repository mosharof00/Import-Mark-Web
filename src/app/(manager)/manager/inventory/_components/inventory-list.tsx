...
export async function InventoryList({ status }: { status: InventoryFilter }) {
  ...
  try {
    let query = supabase
      .from("stock")
      .select(
        "quantity_available, low_stock_threshold, last_updated, products!inner(id, name, sku, status, categories(name), brands(name))"
      )
    ...
    } else if (status === "low_stock" || status === "healthy") {
      ...
      return (
        <div className="border-border bg-card rounded-2xl border p-6 shadow-sm">
          <EmptyState
            ...
            description={
              status === "all"
                ? "Active product stock will appear here."
