...
import { stockHealth } from "@/app/(admin)/admin/inventory/_components/inventory-filters"
...
export async function InventoryStats() {
  ...
  try {
    ...
    for (const row of rows) {
      ...
      if (
        stockHealth(row.quantity_available, row.low_stock_threshold) ===
        "out_of_stock"
      ) {
        outOfStock += 1
      }
