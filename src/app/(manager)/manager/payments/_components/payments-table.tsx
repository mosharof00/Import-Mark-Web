...
function buildColumns(orderHrefBase: string): ColumnDef<PaymentRow>[] {
  return [
    {
      id: "proof",
      header: "Proof",
      cell: ({ row }) =>
        row.original.proofImageUrl ? (
          <a
            href={row.original.proofImageUrl}
            target="_blank"
            rel="noreferrer"
            className="bg-muted block size-12 overflow-hidden rounded-md"
          >
    ...
    },
    {
      ...
      cell: ({ row }) => (
        <div>
          <Link
            href={`${orderHrefBase}/${row.original.orderId}`}
...
export function PaymentsTable({
  ...
  return (
    <div className="border-border bg-card rounded-2xl border p-4 shadow-sm md:p-6">
      <DataTable
        columns={buildColumns(orderHrefBase)}
        data={data}
