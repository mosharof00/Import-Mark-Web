import { CreditCard, Wallet, AlertCircle, Receipt } from "lucide-react"

import { createClient } from "@/lib/supabase/server"
import { getAuthedUser } from "@/lib/auth/get-user"
import { StatCard } from "@/components/shared/stat-card"
import { ErrorCard } from "@/components/shared/error-card"
import { formatTaka } from "@/lib/format"

function monthStart(): string {
  const d = new Date()
  return new Date(d.getFullYear(), d.getMonth(), 1).toISOString().slice(0, 10)
}

export async function PaymentStats() {
  const { user } = await getAuthedUser()
  if (!user) return <ErrorCard title="Couldn't load payment stats" />

  const supabase = await createClient()

  try {
    const { data: orders, error: ordersError } = await supabase
      .from("sales_orders")
      .select("id, due_amount")
      .eq("created_by", user.id)
      .not("status", "in", "(rejected,cancelled)")

    if (ordersError) throw ordersError

    const orderIds = (orders ?? []).map((o) => o.id)
    let collectedThisMonth = 0
    let paymentCount = 0

    if (orderIds.length > 0) {
      const { data: payments, error: paymentsError } = await supabase
        .from("payments")
        .select("amount, payment_date")
        .in("order_id", orderIds)
        .gte("payment_date", monthStart())

      if (paymentsError) throw paymentsError

      paymentCount = payments?.length ?? 0
      collectedThisMonth = (payments ?? []).reduce(
        (sum, p) => sum + p.amount,
        0
      )
    }

    let outstandingDue = 0
    let ordersWithBalance = 0
    for (const order of orders ?? []) {
      const due = order.due_amount ?? 0
      if (due > 0) {
        outstandingDue += due
        ordersWithBalance += 1
      }
    }

    return (
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-4">
        <StatCard
          label="Collected This Month"
          value={formatTaka(collectedThisMonth)}
          icon={CreditCard}
          hint={`${paymentCount} payment${paymentCount === 1 ? "" : "s"}`}
        />
        <StatCard
          label="Outstanding Due"
          value={formatTaka(outstandingDue)}
          icon={Wallet}
          accent={outstandingDue > 0 ? "red" : "default"}
          hint={
            outstandingDue > 0 ? "Across your open orders" : "All collected"
          }
        />
        <StatCard
          label="Orders With Balance"
          value={ordersWithBalance}
          icon={AlertCircle}
          accent={ordersWithBalance > 0 ? "amber" : "default"}
          hint="Need collection"
        />
        <StatCard
          label="Open Orders"
          value={orders?.length ?? 0}
          icon={Receipt}
          hint="Excluding rejected/cancelled"
        />
      </div>
    )
  } catch {
    return <ErrorCard title="Couldn't load payment stats" />
  }
}
